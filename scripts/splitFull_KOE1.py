...
""" Parse the matricemutantFullTab file into 3 main files:
    matricemutantProtTab
    matricemutantKOE1Tab
    matricemutantWTKOE1Tab
@author : Meline WERY
"""
import pandas as pa
import sys, os
...
def MatrixSplitKOE1(filepath):
    """ Function to parse matricemutantFullTab """
    ...
    liste_protein.to_csv(path+'/matricemutantProtTab.csv', sep=',', encoding='utf-8')
    liste_wt = df.drop(df.columns[index_name+1:index_wt], axis=1)
    liste_wtkoe1 = liste_wt.set_index('Name')
    liste_wtkoe1.to_csv(path+'/matricemutantWTKOE1Tab.csv', sep=',', encoding='utf-8')
#
    liste = df.drop(df.columns[index_name+1:index_wt+1], axis=1)
    liste_koe1 = liste.set_index('Name')
    liste_koe1.to_csv(path+'/matricemutantKOE1Tab.csv', sep=',', encoding='utf-8')
