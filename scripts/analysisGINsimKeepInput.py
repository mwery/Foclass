#! /usr/bin/env python3
"""
    Used if we test all combination of inputs and we want to keep them into the value
...
def readGINsimNetworkMutant(fileName, folder):
    ...
    #print(ginsimvers)
    os.system("java -cp %s:extensions/jython-standalone-2.7.0.jar org.ginsim.Launcher -s ./scripts/ginsim/FindStableState.py %s > %s" %(ginsimvers[0], fileName, fileResult))
    #os.system("java -jar %s -s ./scripts/ginsim/FindStableState.py %s > %s" %(ginsimvers[0], fileName, fileResult))
    list_mutant = []
    dict_state = {}
    list_prot = []
                ...
                while(next_line):
                    ...
                    while(next_line.startswith('0') or next_line.startswith('1')):
                        ...
                        if next_line not in dict_state:
                            ...
                            dict_state[next_line].append(mutant)
                        else:
                            value = dict_state[next_line]
                            #print(value)
